...
async function onLogin(user) {
    ...
    myGril = await bot.Contact.find({ name: config.AUTOREPLYPERSON })
    myBot = await bot.Contact.find({ name: config.BOT })
    // 登陆后创建定时任务
    await initDay()
}
...
async function onMessage(msg) {
    ...
    if (room) { // 如果是群消息
        ...
        if (config.AUTOREPLY && contact.name() == config.AUTOREPLYPERSON) { // 如果开启自动聊天且已经指定了智能聊天的对象才开启机器人聊天
            // 10秒内未回复，则使用小冰来回复
            isMyReply = true
            console.log('10秒后未回复，使用小冰来回复')
            myReplyTimer = setTimeout(async _ => {
                if (!isMyReply) return
                myBot = await bot.Contact.find({ name: config.BOT })
                console.log(myBot)
                try {
        ...
        if (contact.name() == config.ME && isMyReply) {
            isMyReply = false
            if (myReplyTimer) clearTimeout(myReplyTimer)
        }
        if (contact.name() == config.BOT && isMyReply) {
            myGril = await bot.Contact.find({ name: config.AUTOREPLYPERSON })
            console.log(myGril)
            try {
                myGril.say(content)
...
// setTimeout(_ => {
//     myGril = bot.Contact.find({ name: config.AUTOREPLYPERSON })
//     myBot = bot.Contact.find({ name: config.BOT })
// }, 1000 * 5)